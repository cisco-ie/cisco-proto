/*
Copyright 2019 Cisco Systems

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by protoc-gen-go. DO NOT EDIT.
// source: snmp_q_stats_b.proto

package cisco_ios_xr_snmp_agent_oper_snmp_information_request_type_detail_nms_addresses_nms_address

import (
	fmt "fmt"
	proto "github.com/golang/protobuf/proto"
	math "math"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

type SnmpQStatsB_KEYS struct {
	NmsAddr              string   `protobuf:"bytes,1,opt,name=nms_addr,json=nmsAddr,proto3" json:"nms_addr,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *SnmpQStatsB_KEYS) Reset()         { *m = SnmpQStatsB_KEYS{} }
func (m *SnmpQStatsB_KEYS) String() string { return proto.CompactTextString(m) }
func (*SnmpQStatsB_KEYS) ProtoMessage()    {}
func (*SnmpQStatsB_KEYS) Descriptor() ([]byte, []int) {
	return fileDescriptor_5af0d739c25f44ca, []int{0}
}

func (m *SnmpQStatsB_KEYS) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_SnmpQStatsB_KEYS.Unmarshal(m, b)
}
func (m *SnmpQStatsB_KEYS) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_SnmpQStatsB_KEYS.Marshal(b, m, deterministic)
}
func (m *SnmpQStatsB_KEYS) XXX_Merge(src proto.Message) {
	xxx_messageInfo_SnmpQStatsB_KEYS.Merge(m, src)
}
func (m *SnmpQStatsB_KEYS) XXX_Size() int {
	return xxx_messageInfo_SnmpQStatsB_KEYS.Size(m)
}
func (m *SnmpQStatsB_KEYS) XXX_DiscardUnknown() {
	xxx_messageInfo_SnmpQStatsB_KEYS.DiscardUnknown(m)
}

var xxx_messageInfo_SnmpQStatsB_KEYS proto.InternalMessageInfo

func (m *SnmpQStatsB_KEYS) GetNmsAddr() string {
	if m != nil {
		return m.NmsAddr
	}
	return ""
}

type SnmpQStatsB struct {
	TotalCount            uint32   `protobuf:"varint,50,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	AgentRequestCount     uint32   `protobuf:"varint,51,opt,name=agent_request_count,json=agentRequestCount,proto3" json:"agent_request_count,omitempty"`
	InterfaceRequestCount uint32   `protobuf:"varint,52,opt,name=interface_request_count,json=interfaceRequestCount,proto3" json:"interface_request_count,omitempty"`
	EntityRequestCount    uint32   `protobuf:"varint,53,opt,name=entity_request_count,json=entityRequestCount,proto3" json:"entity_request_count,omitempty"`
	RouteRequestCount     uint32   `protobuf:"varint,54,opt,name=route_request_count,json=routeRequestCount,proto3" json:"route_request_count,omitempty"`
	InfraRequestCount     uint32   `protobuf:"varint,55,opt,name=infra_request_count,json=infraRequestCount,proto3" json:"infra_request_count,omitempty"`
	XXX_NoUnkeyedLiteral  struct{} `json:"-"`
	XXX_unrecognized      []byte   `json:"-"`
	XXX_sizecache         int32    `json:"-"`
}

func (m *SnmpQStatsB) Reset()         { *m = SnmpQStatsB{} }
func (m *SnmpQStatsB) String() string { return proto.CompactTextString(m) }
func (*SnmpQStatsB) ProtoMessage()    {}
func (*SnmpQStatsB) Descriptor() ([]byte, []int) {
	return fileDescriptor_5af0d739c25f44ca, []int{1}
}

func (m *SnmpQStatsB) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_SnmpQStatsB.Unmarshal(m, b)
}
func (m *SnmpQStatsB) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_SnmpQStatsB.Marshal(b, m, deterministic)
}
func (m *SnmpQStatsB) XXX_Merge(src proto.Message) {
	xxx_messageInfo_SnmpQStatsB.Merge(m, src)
}
func (m *SnmpQStatsB) XXX_Size() int {
	return xxx_messageInfo_SnmpQStatsB.Size(m)
}
func (m *SnmpQStatsB) XXX_DiscardUnknown() {
	xxx_messageInfo_SnmpQStatsB.DiscardUnknown(m)
}

var xxx_messageInfo_SnmpQStatsB proto.InternalMessageInfo

func (m *SnmpQStatsB) GetTotalCount() uint32 {
	if m != nil {
		return m.TotalCount
	}
	return 0
}

func (m *SnmpQStatsB) GetAgentRequestCount() uint32 {
	if m != nil {
		return m.AgentRequestCount
	}
	return 0
}

func (m *SnmpQStatsB) GetInterfaceRequestCount() uint32 {
	if m != nil {
		return m.InterfaceRequestCount
	}
	return 0
}

func (m *SnmpQStatsB) GetEntityRequestCount() uint32 {
	if m != nil {
		return m.EntityRequestCount
	}
	return 0
}

func (m *SnmpQStatsB) GetRouteRequestCount() uint32 {
	if m != nil {
		return m.RouteRequestCount
	}
	return 0
}

func (m *SnmpQStatsB) GetInfraRequestCount() uint32 {
	if m != nil {
		return m.InfraRequestCount
	}
	return 0
}

func init() {
	proto.RegisterType((*SnmpQStatsB_KEYS)(nil), "cisco_ios_xr_snmp_agent_oper.snmp.information.request_type_detail.nms_addresses.nms_address.snmp_q_stats_b_KEYS")
	proto.RegisterType((*SnmpQStatsB)(nil), "cisco_ios_xr_snmp_agent_oper.snmp.information.request_type_detail.nms_addresses.nms_address.snmp_q_stats_b")
}

func init() { proto.RegisterFile("snmp_q_stats_b.proto", fileDescriptor_5af0d739c25f44ca) }

var fileDescriptor_5af0d739c25f44ca = []byte{
	// 267 bytes of a gzipped FileDescriptorProto
	,
}
